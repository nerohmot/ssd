...
# -*- coding: utf-8 -*-
#
# This is the configuration file for the Spyder Spinner Daemon
#
#
# Remember to restart the Spyder Spinner Deamon (ssd) after making changes to
...
#
guest_can_manage_environments = yes

##############################################################################
# The `max_slots` keyword determines the maximum number of processes `ssd`
# can spin up. This is a (posetive) integer value. If the value can't
# be interpreted, the fallback is '0'. '0' means automatic calculation, it
# takes the number of CPU cores, deducts 1 and that is set. 
# There is in principle no upper limit ...
#
# Notes:
#  * `ssd` will publish the number of available slots (= max_slots - used_slots)
#  * AUTO means number of (logical) CPU cores - 1
#  * anything starting with 'auto' (regardless capitalization) is considered 'AUTO'
#  * `CORES` is a constant that equates to the number of (logical) CPU cores
...
#
# Notes:
#  * It **must** be a python style list
