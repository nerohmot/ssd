...
class ConnectionWizard(QtWidgets.QDialog):
    ...
    def __init__(self, listener):
        ...
        host_list = ["local", "Carlos' Mac Book Pro", "Ralf's Server", "Gonzalo's Raspberry PI", "Tom's MiniSCT"]
        self.spyderHosts.blockSignals(True)
        self.spyderHosts.clear()
        if host_list != []:
            self.spyderHosts.addItems(host_list)
            self.spyderHosts.setCurrentText(host_list[0])  # = localhost if available
            self.spyderHosts.setSizeAdjustPolicy(QtWidgets.QComboBox.AdjustToContents)
        self.spyderHosts.setEnabled(True)
        self.spyderHosts.blockSignals(False)
